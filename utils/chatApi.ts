...
import { saveRemotePublicKey } from './encryption';
...
export function useChatApi() {
  const { initEncryption, decryptText, publicKeyCache } = useEncryption();
    ...
    const messages = await Promise.all(
      snap.docs.map(async (d) => {
        ...
        if (data.publicKey && data.senderId) {
          await saveRemotePublicKey(data.senderId, data.publicKey, publicKeyCache);
          return null;
        }

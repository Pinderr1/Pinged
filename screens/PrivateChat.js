...
function PrivateChat({ user, initialGameId }) {
  ...
  const handleSend = () => {
    if (text.trim()) {
      sendMessage({ matchId: user.id, text });
      setText('');
      updateTyping(false);
    }
  };
  ...
  const handleGameEnd = (result) => {
    ...
    if (result.winner !== undefined) {
      ...
      sendMessage({ matchId: user.id, text: `Game over. ${msg}`, meta: { senderId: 'system' } });
      if (result.winner === '0') {
        playSound('win');
      ...
      sendMessage({ matchId: user.id, text: 'Game over. Draw.', meta: { senderId: 'system' } });
      playSound('draw');
  ...
  const handleGameSelect = (gameId) => {
    ...
    if (activeGameId && activeGameId !== gameId) {
      sendMessage({ matchId: user.id, text: `Switched game to ${title}`, meta: { senderId: 'system' } });
    } else if (!activeGameId) {
      sendMessage({ matchId: user.id, text: `Game started: ${title}`, meta: { senderId: 'system' } });
      recordGamePlayed();
    }
